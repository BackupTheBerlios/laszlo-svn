...
public class LineReadableInputStream extends InputStream {
	...
	public int read(byte[] b, int off, int len) throws IOException {
		if (prereadLen > 0) {
			if (prereadLen < len) {
				System
						.arraycopy(prereadBytes, prereadStart, b, off,
								prereadLen);
	...
	public String readLine() throws IOException {
		...
		stringReading: while (true) {
			...
			for (int p = 0; p < prereadLen; p++) {
				if (prereadBytes[prereadStart + p] == lineSeparator[lineSeparatorMatchLength]) {
					...
					if (lineSeparatorMatchLength == lineSeparator.length) {
						p++;
						bytesForString = extendByteArray(bytesForString,
								prereadBytes, prereadStart, p);
						prereadStart += p;
						prereadLen -= p;
						break stringReading;
					}
			...
			bytesForString = extendByteArray(bytesForString, prereadBytes,
					prereadStart, prereadLen);
			prereadLen = 0;
		}
		...
		return new String(bytesForString, 0, bytesForString.length
				- lineSeparator.length, "US-ASCII");
	}
	...
	public void pushback(byte b) throws IOException {
		if (prereadLen > 0) {
			if (prereadStart > 0) {
				...
				System.arraycopy(prereadBytes, 0, temp, PREREAD_BUFFER,
						prereadBytes.length);
	...
	public void pushbackAfterPreread(byte b) throws IOException {
		if (prereadLen > 0) {
			if (prereadStart >= 0
					&& prereadStart + prereadLen < prereadBytes.length) {
				prereadBytes[prereadStart + prereadLen] = b;
				prereadLen++;
			} else if (prereadStart > 0) {
				System.arraycopy(prereadBytes, prereadStart, prereadBytes,
						prereadStart - 1, prereadLen);
				prereadStart--;
				prereadBytes[prereadStart + prereadLen] = b;
				prereadLen++;
			} else {
	...
	private byte[] extendByteArray(final byte[] oldBytes, byte[] newBytes,
			int newBytesStart, int newBytesLen) {
		if (oldBytes == null) {
			...
			System.arraycopy(newBytes, newBytesStart, temp, oldBytes.length,
					newBytesLen);
			return temp;
		}
	}
