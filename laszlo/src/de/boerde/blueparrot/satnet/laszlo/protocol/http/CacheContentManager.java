 ...
 * @author  roland
 */
public class CacheContentManager implements ContentManager, PackageManager.PackageListener
{
				...
				if ((subdir != null) && subdir.isDirectory())
				{
					...
					if (hostdirs != null)
					{
						for (int h=0; h<hostdirs.length; h++)
						{
							...
							String hostdirName = hostdir.getName().toLowerCase();
							Vector hostdirVector = (Vector) hostdirMap.get (hostdirName);
	...
	public void packageUpdated (PackageManager.PackageInfo oldInfo, PackageManager.PackageInfo newInfo)
	{
		synchronized (hostdirMap)
		{
	...
	public ContentManagerInfo get (String uri)
	{
			...
			synchronized (hostdirMap)
			{
				...
				if (hostdirVector != null)
				{
					for (int v=0; v<hostdirVector.size(); v++)
					{
						...
						try
						{
							File testFile = new File (hostdirAbsolutePath + File.separatorChar + localUri.substring (hostName.length()+1));
							testFile = testFile.getCanonicalFile();
							String testFileAbsolutePath = testFile.getAbsolutePath();
							if (testFileAbsolutePath.startsWith (hostdirAbsolutePath))
							{	// make sure we stay in the correct host directory and do not navigate out of it via any ".." or similar constructions, to prevent javascript attacks
								int hostdirAbsolutePathLen = hostdirAbsolutePath.length();
								if ((testFileAbsolutePath.length() == hostdirAbsolutePathLen) || (testFileAbsolutePath.charAt (hostdirAbsolutePathLen) == File.separatorChar))
								{
									if (testFile.exists())
									{
										resultFile = testFile;
		...
		if (resultFile.isDirectory ())
		{
			...
			{
				fileResultFile = new File (resultFile.getAbsolutePath() + File.separator + welcomeNames [w]);
...
//		assert uri.startsWith ("http://");
	...
	static String rewriteUriToLocal (String uri, int iteration)
	{
		...
		// convert the hostname part to lowercase (our way to assure case-insensitivity...
		int endOfHostname = uriBuffer.indexOf ("/");
		if (endOfHostname < 0)
			endOfHostname = uriBuffer.length();
		uriBuffer.replace (0, endOfHostname, uriBuffer.substring (0, endOfHostname).toLowerCase());
...
//System.out.println ("#iter" + iteration + " ambig " + charsAmbiguous + ": nullExit " + uriBuffer.toString());
	...
	private static final String[] welcomeNames = { "index.htm", "index.html", "welcome.htm", "welcome.html" };
	private static final String[] welcomeTypes = { "text/html", "text/html", "text/html", "text/html" };
