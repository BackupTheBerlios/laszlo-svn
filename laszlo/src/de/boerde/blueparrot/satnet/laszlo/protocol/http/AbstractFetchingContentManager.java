...
import de.boerde.blueparrot.io.LineReadableInputStream;
...
abstract public class AbstractFetchingContentManager implements Producer {
	public void doHead(RequestInfo request, ResponseInfo response)
			throws IOException {
	...
	public void doPost(RequestInfo request, ResponseInfo response)
			throws IOException {
		service(request, response);
	}

	public void service(RequestInfo request, ResponseInfo response)
			throws IOException {
		...
		do {
			...
			Writer out = new OutputStreamWriter(outStream, "CP1252"); // try
																		// to be
																		// 8-bit
																		// clean
																		// by
			...
			while (headers.hasNext()) {
				...
				if (colonPos > 0) {
					...
					if ((headerName.equalsIgnoreCase("Proxy-Connection"))
						...
						requestBodyLength = Integer.parseInt(request
		...
		if ("POST".equalsIgnoreCase(method)) {
			...
			while (true) {
				int count = clientIn.read(bytes, 0,
						requestBodyLength > bytes.length ? bytes.length
								: requestBodyLength);
				if (count < 0)
					break;
		...
		while (true) {
			...
			if (colonPos > 0) {
				...
				if (checkRemoteResponsetHeader(connInfo, headerName,
						headerValue)) {
		...
		if (!"HEAD".equalsIgnoreCase(method) && (status != 304)
				&& (status != 204) && (status >= 200)) {
			while (bodyLength != 0) {
				int count;
				if (bodyLength > 0) {
					count = in.read(bytes, 0,
							bodyLength > bytes.length ? bytes.length
									: bodyLength);
					bodyLength -= count;
				} else {
					count = in.read(bytes);
	...
	abstract protected void releaseConnection(ConnectionInfo connInfo)
			throws IOException;

	abstract protected ConnectionInfo getConnection(RequestInfo request)
			throws IOException;

	abstract protected void addRemoteRequestHeaders(ConnectionInfo info,
			Writer output) throws IOException;

	abstract protected boolean checkRemoteResponsetHeader(ConnectionInfo info,
	...
	protected static class ConnectionInfo {
		...
		protected ConnectionInfo(SocketChannel socketChannel)
				throws IOException {
