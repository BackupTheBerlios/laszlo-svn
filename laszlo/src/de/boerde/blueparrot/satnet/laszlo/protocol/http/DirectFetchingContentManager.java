 ...
 * @author  roland
 */
public class DirectFetchingContentManager extends AbstractFetchingContentManager implements Producer
{
	...
	protected ConnectionInfo getConnection (RequestInfo request) throws IOException
	{
		...
		if (pos > 0)
		{
			...
			if (port <= 0)
				throw new NumberFormatException ("Negative Port numbers not allowed");
		}
		else
		{
			host = hostAndPort;
		...
		{
			Vector hostConnections = (Vector) openConnections.get (socketAddress);
			...
			else
			{
				...
				} while ((connection.getTimeoutTime() <= now) && !hostConnections.isEmpty());
	...
	protected boolean checkRemoteResponsetHeader (ConnectionInfo info, String headerName, String headerValue)
	{
		if (headerName.equalsIgnoreCase ("Connection"))
		{
			if (headerValue.equalsIgnoreCase ("keep-alive"))
			{
				...
				{
					tinfo.setTimeoutTime (System.currentTimeMillis() + 30 * 1000);
				}
			}
			else
			{
				...
				if (pos > 0)
				{
					...
					if ("timeout".equalsIgnoreCase (name))
					{
						try
						{
							...
							{
								TimeoutConnectionInfo tinfo = (TimeoutConnectionInfo) info;
								tinfo.setTimeoutTime (System.currentTimeMillis() + timeout * 1000);
							}
						}
	...
	private void checkAllTimeouts()
	{
		synchronized (openConnections)
		{
			...
			while (addresses.hasMoreElements())
			{
				...
				{
					TimeoutConnectionInfo info = (TimeoutConnectionInfo) hostConnections.get (c);
	...
	protected class TimeoutConnectionInfo extends ConnectionInfo
	{
		...
		protected TimeoutConnectionInfo (SocketChannel socketChannel) throws IOException
		{
			super (socketChannel);
		}
