 ...
 * @author roland
 */
public class WebcastProviderCategoryListGenerator extends HtmlGenerator
		implements Producer {
	private String name;
	...
	public WebcastProviderCategoryListGenerator(String name, String title,
	...
	public void doGet(RequestInfo request, ResponseInfo response)
		...
		String[] refresh = { "<meta http-equiv=\"Refresh\" content=\"60;url=http://"
				+ settings.getHttpOwnPseudoName() + "/" + name + "\" />" };
		...
		if (categories != null) {
			...
			synchronized (categories) {
				...
				while (catEnum.hasMoreElements()) {
					...
					for (int s = 0; s < wantedServices.length; s++) {
						if (categoryHasService(categories, category,
								wantedServices[s])) {
	...
	private boolean categoryHasService(Hashtable categories, String category,
			String service) {
	...
	/*
	 * private boolean categoryHasServiceOtherThan (Hashtable categories, String
	 * category, String service) { Vector categorysPackages = (Vector)
	 * categories.get (category); Iterator packages =
	 * categorysPackages.iterator(); while (packages.hasNext()) {
	 * PackageManager.PackageInfo info = (PackageManager.PackageInfo)
	 * packages.next(); if (!service.equals
	 * (info.getXmlAnnouncement().getServiceName())) { return true; } } return
	 * false; }
	 */
