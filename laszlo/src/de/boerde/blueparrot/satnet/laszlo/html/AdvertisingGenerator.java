...
public class AdvertisingGenerator extends HtmlGenerator implements Producer
{
	...
	public void doGet (RequestInfo request, ResponseInfo response) throws IOException
	{
		...
		String[] refresh = { "<meta http-equiv=\"Refresh\" content=\"120;url=http://" + settings.getHttpOwnPseudoName() + "/advertising\" />" };
		...
		{
			PackageManager.PackageInfo info = (PackageManager.PackageInfo) iterator.next();
			BookingAnnouncement xmlAnnouncement = info.getXmlAnnouncement();
			if ("advertising".equalsIgnoreCase (info.getXmlAnnouncement().getServiceName()))
			{
		...
		while (size > 0)
		{
			...
			{
				linkUrl = "/ads_received/" + indexHtml.substring ("http://".length());
			}
			else
			{
			...
			receivedAdsGenerator.addAdvertisingHostdir (indexHtml.substring ("http://".length(), hostdirEnd));

			CacheContentManager cacheManager = CacheContentManager.getCacheContentManager();
		...
		if (info != null)
		{
			...
			try
			{
				...
				if (!"".equals (rich))
				{
					out.print ("window.open (\"" + clickUrl + rich + "\", \"satelliteAdvertising\", \"");
					boolean comma=false;
/*					if (border != null)
					{
						if (comma)
 ...
 */
				...
				for (int b=0; b<banners.getLength(); b++)
				{
					...
					if (!(bannerNode instanceof Element))
						break;
					banner = bannerDir + XMLHelpers.getContentText ((Element) bannerNode);

					out.println ("<iframe src=\"" + banner + "\" frameborder=\"0\" scrolling=\"no\" marginwidth=\"0\" marginheight=\"0\" width=\"468\" height=\"60\">");
