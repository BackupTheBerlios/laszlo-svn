...
import de.boerde.blueparrot.satnet.laszlo.ProviderNames;
 ...
 * @author roland
 */
public class WebcastProviderCategoryPackageListGenerator extends HtmlGenerator
		implements Producer {
	private String name;
	...
	public WebcastProviderCategoryPackageListGenerator(String name,
			String category, String caster, Vector packages) {
		this.name = name;
		...
		if (packages != null) {
			synchronized (packages) {
				...
				while (packEnum.hasMoreElements()) {
					PackageManager.PackageInfo info = (PackageManager.PackageInfo) packEnum
							.nextElement();
					...
					if (index != null) {
						...
						// Problem:
						// The index file that the announcement points to
						// sometimes does not exist.
						CacheContentManager contentManager = CacheContentManager
								.getCacheContentManager();
						link = (contentManager.get(index) != null) ? index
								: url;
					} else {
						link = url;
					}
					...
					out
							.print(dateFormat.format(
									new Date(announcement.getUpdatedTime()))
									.toString());
					out.println("</p>");
					out.println("<p>");
