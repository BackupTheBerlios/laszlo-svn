...
public class Extractor {
	...
	public boolean extract(String localFile, File destDir)
			throws java.io.IOException {
		Runtime runtime = Runtime.getRuntime();
		Settings settings = Settings.getSettings();
		StringBuffer command = new StringBuffer(settings
				.getUnpackCommandline(localFile));
		for (int pos = command.indexOf("%%PACKEDFILE%%"); pos >= 0; pos = command
				.indexOf("%%PACKEDFILE%%", pos)) {
			command.delete(pos, pos + "%%PACKEDFILE%%".length());
			command.insert(pos, localFile);
		}
		for (int pos = command.indexOf("%%DESTDIR%%"); pos >= 0; pos = command
				.indexOf("%%DESTDIR%%", pos)) {
		...
		/*
		 * System.out.print ("###"); for (int i=0; i<commandArr.length; i++) {
		 * System.out.print (commandArr [i]); System.out.print ("!"); }
		 * System.out.println();
		 */
		...
		try {
			/*
			 * { String lastLine = ""; BufferedReader reader = new
			 * BufferedReader (new InputStreamReader
			 * (process.getInputStream())); while (true) { String line =
			 * reader.readLine(); if (line == null) break;
			 * 
			 * lastLine = line; if (line.equals ("All OK")) { success = true; } }
			 * System.out.println (localFile + " ###LastLine--> " + lastLine); }
			 */
			process.waitFor();
			File sourceFile = new File(localFile);
			sourceFile.delete();
			// System.out.println (localFile + " ###exitValue " +
			// process.exitValue());
			success = (process.exitValue() == 0);
			if (!success) {
				GUIMain.getLogger().warning(
						"Error while unpacking " + localFile);
