 ...
 * @author roland
 */
public class Receiver extends Thread implements
		ContentReader.TransmissionListener {
	...
	public void run() {
		MulticastSocketPool multicastSocketPool = MulticastSocketPool
				...
				if (shouldReceive.getResult()) {
					...
					try {
						ReceiverContentReaderThread t = new ReceiverContentReaderThread(
								a);
						ReceiverContentReader contentReader = t
								.getContentReader();
	...
	private synchronized void notifyNewTransmission(Announcement announcement,
			ContentReader reader) {
		NewTransmissionEvent evt = new NewTransmissionEvent(announcement,
				reader);
		Iterator listeners = ((Vector) newTransmissionListeners.clone())
				.iterator();
		while (listeners.hasNext()) {
			NewTransmissionListener listener = (NewTransmissionListener) listeners
					.next();
	...
	public void transmissionExpired(
			de.boerde.blueparrot.satnet.laszlo.ContentReader.TransmissionEvent evt) {
		removeContentReader(evt.getContentReader());
	}
	...
	private class SaveHook extends Thread {
		...
		public void run() {
			...
			GUIMain.getLogger().info("hook: finish");
		}
	}
}
