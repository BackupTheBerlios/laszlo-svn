...
import de.boerde.blueparrot.util.FileFinder;
...
public class ContentReader {
	...
	public ContentReader(Announcement announcement,
			BookingAnnouncement xmlAnnouncement, int received,
			ForwardBitChunkList partsToRetrieve) throws ProtocolException {
		...
		try {
			...
			totalNumberOfPackets = (tsize % blksize == 0) ? tsize / blksize
					: tsize / blksize + 1;
		} catch (Exception e) {
	...
	protected String dataDump(byte[] data, int num) {
		...
		for (int i = 0; i < data.length && i < num; i++) {
			...
			buf.append(" ").append(hexDigits[upperNibble]).append(
					hexDigits[lowerNibble]);
		}
		return buf.toString();
	}
	...
	private StringBuffer appendLocalFileName(StringBuffer toAppendTo) {
		StringBuffer name = (toAppendTo != null) ? toAppendTo
				: new StringBuffer();
		int firstIndex = name.length();
		name.append(announcement.getFullName()
	...
	private String getProgressFileName() {
		return appendLocalFileName(new StringBuffer()).append(progressSuffix)
				.toString();
	}

	protected void savePartsToRetrieve() {
		String fileid = announcement.getDetail("fileid");
		if (fileid == null) { // no saving of progress information if file
								// does not have an id.
			return;
		}
		...
		try {
			...
			// GUIMain.logger.info("Saved progress information for file " +
			// announcement.getFullName() + " id " + fileid + " tsize " + tsize
			// + " blksize " + announcement.getDetail ("blksize") + " received "
			// + received + " partsToRetrieve " + partsToRetrieve);
		} catch (IOException e) {
	...
	protected void loadPartsToRetrieve() {
		String fileid = announcement.getDetail("fileid");
		if (fileid == null) { // no loading of progress information if file
								// does not have an id.
			return;
		}
		File progressFile = new File(getProgressFileName());
		if (!progressFile.exists()) { // nothing to do if there is no progress
										// file.
			return;
		}
		...
		try {
			...
			if (announcement.getFullName().equals(
					savedAnnouncement.getFullName())
					&& announcement.getDetail("fileid").equals(
							savedAnnouncement.getDetail("fileid"))
					&& announcement.getDetail("blksize").equals(
							savedAnnouncement.getDetail("blksize"))
					&& announcement.getDetail("tsize").equals(
							savedAnnouncement.getDetail("tsize"))) {
				BookingAnnouncement tempXmlAnnouncement = (BookingAnnouncement) in
						.readObject();
				int tempReceived = in.readInt();
				ForwardBitChunkList tempPartsToRetrieve = (ForwardBitChunkList) in
						.readObject();
				xmlAnnouncement = tempXmlAnnouncement;
				received = tempReceived;
				partsToRetrieve = tempPartsToRetrieve;
				// GUIMain.logger.info("Loaded progress information for file " +
				// announcement.getFullName() + " id " + fileid + " tsize " +
				// tsize + " blksize " + announcement.getDetail ("blksize") + "
				// received " + received + " partsToRetrieve " +
				// partsToRetrieve);
			}
		} catch (IOException e) {
			GUIMain.getLogger().severe(e.getMessage());
			...
			GUIMain.getLogger().warning(
					"File " + announcement.getFullName() + " id " + fileid
	...
	protected synchronized void notifyTransmissionCompleted() {
		...
		Iterator listeners = ((Vector) transmissionListeners.clone())
				.iterator();
		while (listeners.hasNext()) {
			TransmissionListener listener = (TransmissionListener) listeners
					.next();
	...
	private static ContentReader loadAnonymousProgressFile(File file) {
		...
		try {
			...
			Announcement savedAnnouncement = (Announcement) in.readObject();
			BookingAnnouncement savedXmlAnnouncement = (BookingAnnouncement) in
					.readObject();
			int savedReceived = in.readInt();
			ForwardBitChunkList savedPartsToRetrieve = (ForwardBitChunkList) in
					.readObject();
			result = new ContentReader(savedAnnouncement, savedXmlAnnouncement,
					savedReceived, savedPartsToRetrieve);
		} catch (Throwable e) {
			file.delete();
	...
	private static void expireContentReaders() {
		Settings settings = Settings.getSettings();
		long decideTime = System.currentTimeMillis()
				- (60 * 1000 * settings.getExpireIncompleteFileAfterMinutes());
		synchronized (allContentReaders) {
			...
			while (iter.hasNext()) {
				...
				if (reader.isDone()) {
					...
					if ((progressFile.lastModified() < decideTime)
							&& (file.lastModified() < decideTime)) {
						iter.remove();
						file.delete();
						progressFile.delete();
	...
	private static void expireFiles() {
		Settings settings = Settings.getSettings();
		final long decideTime = System.currentTimeMillis()
				- (60 * 1000 * settings.getExpireIncompleteFileAfterMinutes());
		File dir = new File(settings.getWorkDirectory() + File.separatorChar
				+ "recv");
	...
	protected void expireSimilarContentReaders() {
		...
		synchronized (allContentReaders) {
			...
			while (iter.hasNext()) {
				ContentReader reader = (ContentReader) iter.next();
				BookingAnnouncement otherXmlAnnouncement = reader
						.getXmlAnnouncement();
	...
	protected static void addContentReaderToAll(ContentReader reader) {
		synchronized (allContentReaders) {
			...
			for (int c = 0; c < allContentReaders.size(); c++) {
				ContentReader existing = (ContentReader) allContentReaders
						.get(c);
						...
						if (file.getName().endsWith(progressSuffix)) {
							...
							if (reader != null) {
								...
								for (pos = 0; pos < result.size(); pos++) {
									File fileAtPos = (File) resultFiles
											.get(pos);
									if (fileLastModified < fileAtPos
											.lastModified())
										break;
								}
	...
	private static class ExpiryThread extends Thread {
		...
		public synchronized void run() {
			while (true) {
				try {
					Settings settings = Settings.getSettings();
					wait(60 * 1000 * settings
							.getExpireIncompleteFileCheckIntervalMinutes());
					expireContentReaders();
					expireFiles();
				} catch (Throwable t) {
					GUIMain.getLogger().severe(t.getMessage());
				}
			}
		}
	}
