...
public class PackageManager {
	...
	public void setPackageInfo(File dir, Announcement announcement,
			BookingAnnouncement xmlAnnouncement) {
	...
	private void removePackageDir(File topDir) {
		...
		File infoFile = new File(topDir.getAbsolutePath() + File.separator
				+ "laszloinfo.ser");
		infoFile.delete();
	...
	private static PackageInfo readPackageInfo(File dir) {
		...
		try {
			if (file.exists()) {
				...
				BookingAnnouncement xmlAnnouncement = (BookingAnnouncement) in
						.readObject();
			...
			GUIMain.getLogger().severe("Invalid package information in " + dir);
		} catch (Exception e) {
			GUIMain.getLogger().severe(e.getMessage());
			info = null;
		} finally {
	...
	public File getUniqueSubdir() {
		synchronized (dirNumber) {
			...
			do {
				...
				dir = new File(workDir + File.separator + "pkgs"
						+ File.separator + dirNumber.toString());
			} while (dir.exists());
			dir.mkdirs();
			return dir;
		}
	...
	private void readOldPackageInfo() {
		...
		long oldestAllowedTime = System.currentTimeMillis()
				- (settings.getExpirePackageAfterMinutes() * 60 * 1000);
		...
		if (subdirs != null) {
			for (int i = 0; i < subdirs.length; i++) {
				...
				if ((subdir != null) && subdir.isDirectory()) {
					...
					if (info != null) {
						long updatedTime = info.getXmlAnnouncement()
								.getUpdatedTime();
						if (updatedTime >= oldestAllowedTime) {
							BookingAnnouncement announcement = info
									.getXmlAnnouncement();
							packagesByUrl.put(announcement.getUrl(), info);
						} else {
							GUIMain.getLogger().info(
									"Expire: "
											+ info.getXmlAnnouncement()
													.getUrl());
		...
		public void run() {
			while (true) {
				...
				try {
					Thread
							.sleep(settings
									.getExpirePackageCheckIntervalMinutes() * 60 * 1000);
				...
				synchronized (packagesByUrl) {
					...
					while (packages.hasMoreElements()) {
						...
						long packageTime = info.getXmlAnnouncement()
								.getUpdatedTime();
						if (packageTime < oldestAllowedTime) {
							...
							Iterator iter = listeners.iterator();
							while (iter.hasNext()) {
								PackageListener listener = (PackageListener) iter
										.next();
	...
	public static class PackageInfo {
		...
		protected PackageInfo(File dir, Announcement announcement,
				BookingAnnouncement xmlAnnouncement) {
			this.dir = dir;
