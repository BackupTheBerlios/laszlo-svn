 ...
 * @author roland
 */
public class ReceptionProcessor implements Receiver.NewTransmissionListener,
	...
	public void transmissionCompleted(ContentReader.TransmissionEvent evt) {
		...
		if (localFile.endsWith("webcasters.xml")) {
		...
		} else if (localFile.endsWith(".xml")
				&& (localFile.indexOf(File.separator + "Announcement"
						+ File.separator) >= 0)) {
			...
			Thread t = new ExtractorBuilder(localFile, announcement,
					xmlAnnouncement);
			t.start();
		}
	}
	...
	private static class ExtractorBuilder extends Thread {
		...
		public ExtractorBuilder(String localFile, Announcement announcement,
		...
		public void run() {
			try {
				...
				if (success) {
					packageManager.setPackageInfo(dir, announcement,
							xmlAnnouncement);
				}
			} catch (Exception e) {
