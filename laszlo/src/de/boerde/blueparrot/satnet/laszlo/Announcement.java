...
public class Announcement implements Serializable {
	...
	private void decodeDatagram(DatagramPacket packet) throws IOException {
		...
		if ((data[offset] != (byte) 0) || (data[offset + 1] != (byte) 2)) {
			GUIMain.getLogger().warning(
					"First two bytes of datagram are not 0x00 0x02.");
		}
		...
		for (int pos = 2; pos < length; pos++) {
			...
			if (current == (byte) 0) {
				String str = new String(data, startPos, currentPos - startPos,
						charset);
				...
				case FIELD_MUSTBEOCTET: {
					...
					if (!str.equals("octet")) {
						GUIMain
								.getLogger()
								.warning(
										"second field is not 'octet'. My protocol implementation may not cope with that.");
					}
	...
	private static boolean areDetailsEqual(Hashtable h1, Hashtable h2,
			String detailName) {
	...
	public boolean equals(Object o) {
		...
		if (((fullName == null && other.fullName == null) || ((fullName != null) && fullName
				.equals(other.fullName)))
				&& ((mustBeOctet == null && other.mustBeOctet == null) || ((mustBeOctet != null) && mustBeOctet
						.equals(other.fullName)))) {
