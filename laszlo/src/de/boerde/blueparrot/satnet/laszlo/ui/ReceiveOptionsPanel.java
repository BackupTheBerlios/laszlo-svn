 ...
 * @author  roland
 */
public class ReceiveOptionsPanel extends JPanel implements OptionsPanel, ItemListener, DocumentListener, ChangeListener
{
	...
	public ReceiveOptionsPanel (OptionsWindow window)
	{
		...
		announcementPortField = new JSpinner (new SpinnerNumberModel (settings.getAnnouncementPort(), 0, 65535, 1));
		announcementPortField.setEditor (new JSpinner.NumberEditor (announcementPortField, "#####0"));
		...
		receiveUseMulticastPoolField.setSelected (settings.getReceiveUseMulticastPool());
	...
	public void applySettings()
	{
		...
		settings.setReceiveUseMulticastPool (receiveUseMulticastPoolField.isSelected());
	}
	...
	public void revertSettings ()
	{
		...
		localInterfaceField.setNetworkInterface (settings.getDVBInterface());
		multicastAddressField.setInetAddress (settings.getMulticastIP());
		announcementPortField.setValue (new Integer (settings.getAnnouncementPort()));
		receiveUseMulticastPoolField.setSelected (settings.getReceiveUseMulticastPool());
	}
	...
	{
		Settings settings = Settings.getSettings();
		return !(settings.getDVBInterface() != null && settings.getDVBInterface().equals (localInterfaceField.getNetworkInterface())
			&& settings.getMulticastIP().equals (multicastAddressField.getInetAddress())
			&& (((Number) announcementPortField.getValue()).intValue() == settings.getAnnouncementPort ())
			&& (settings.getReceiveUseMulticastPool() == receiveUseMulticastPoolField.isSelected())
			);
	}
