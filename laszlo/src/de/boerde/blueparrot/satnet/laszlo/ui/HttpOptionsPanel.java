 ...
 * @author  roland
 */
public class HttpOptionsPanel extends JPanel implements OptionsPanel, ItemListener, DocumentListener, ChangeListener
{
	...
	public HttpOptionsPanel (OptionsWindow window)
	{
		...
		upstreamProxyHostField = new JTextField (settings.getUpstreamHttpProxyHost());
		...
		upstreamProxyPortField = new JSpinner (new SpinnerNumberModel (settings.getUpstreamHttpProxyPort(), 1, 65535, 1));
		upstreamProxyPortField.setEditor (new JSpinner.NumberEditor (upstreamProxyPortField, "#####0"));
		...
		ownPortField = new JSpinner (new SpinnerNumberModel (settings.getLaszloHttpPort(), 1, 65535, 1));
	...
	public void applySettings()
	{
		...
		settings.setUpstreamHttpProxyHost (upstreamProxyHostField.getText().trim());
		Object upstreamProxyPortVal = upstreamProxyPortField.getValue();
		if ((upstreamProxyPortVal != null) && (upstreamProxyPortVal instanceof Number))
			settings.setUpstreamHttpProxyPort (((Number) upstreamProxyPortVal).intValue());
	}
	...
	public void revertSettings ()
	{
		...
		upstreamProxyPortField.setValue (new Integer (settings.getUpstreamHttpProxyPort()));
	}

	public boolean isChangeInUI ()
	{
		Settings settings = Settings.getSettings();
		return !(settings.getLaszloHttpBindAddress ().equals (bindAddressField.getInetAddress())
			&& (((Number) ownPortField.getValue()).intValue() == settings.getLaszloHttpPort ())
			&& ownNameField.getText().trim().equalsIgnoreCase (settings.getHttpOwnPseudoName())
			&& (fetchModeField.getMode().charAt(0) == settings.getHttpProxyFetching().charAt (0))
			&& upstreamProxyHostField.getText().trim().equalsIgnoreCase (settings.getUpstreamHttpProxyHost())
			&& (((Number) upstreamProxyPortField.getValue()).intValue() == settings.getUpstreamHttpProxyPort ()));
	}
