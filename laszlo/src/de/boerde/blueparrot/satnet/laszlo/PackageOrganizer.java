...
public class PackageOrganizer
{
	...
	PackageOrganizer()
	{
		...
		htmlManager.setPath ("providers", new WebcastProviderListGenerator ("providers", webcasters));
	}
	...
	synchronized void initialize (PackageManager packageManager)
	{
		...
		do
		{
			try
			{
				...
				{
					PackageManager.PackageInfo info = (PackageManager.PackageInfo) allPackages.next();
					setPackageInfo (info);
				}
			}
...
/*		String category = announcement.getOneValue ("PROFILE/CATEGORY/CATEGORY_NAME");
...
*/
		...
		synchronized (webcasters)
		{
			...
			if (categories == null)
			{
				...
				htmlManager.setPath (webName, new WebcastProviderCategoryListGenerator (webName, webcaster, categories));
			}
		}
		Vector packages;
		...
		synchronized (packages)
		{
			...
			{
				PackageManager.PackageInfo otherInfo = (PackageManager.PackageInfo) packages.get (pos);
//				int comp = url.compareToIgnoreCase (otherInfo.getXmlAnnouncement().getUrl());
		...
		synchronized (webcasters)
		{
			...
			synchronized (categories)
			{
				...
				synchronized (packages)
				{
					...
					{
						PackageManager.PackageInfo otherInfo = (PackageManager.PackageInfo) packages.get (pos);
						if (url.equalsIgnoreCase (otherInfo.getXmlAnnouncement().getUrl()))
						{
							packages.remove (pos);
							break;
						}
