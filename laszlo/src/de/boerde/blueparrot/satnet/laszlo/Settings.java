...
public class Settings extends Properties
{
		...
		if (dvbInterfaceSetting == null)
		{
			try
			{
				...
				while (ifaces.hasMoreElements())
				{
					...
					else if ((quality < 4) && (displayName.indexOf ("dvb") > -1))
					{
						dvbInterface = iface;
						quality = 4;
					}
					else if ((quality < 3) && (displayName.indexOf ("sat") > -1))
					{
						dvbInterface = iface;
						quality = 3;
					}
					else if ((quality < 2) && (displayName.indexOf ("b2c2 broadband receiver") > -1))
					{
						dvbInterface = iface;
						quality = 2;
					}
	...
	public void setWorkDirectory (String workDirectory)
	{
		...
		while (workDirectory.endsWith (File.separator))
			workDirectory = workDirectory.substring (0, workDirectory.length()-1);
	...
	public String getUnpackCommandline (String file)
	{
		...
		while (command == null || "".equals (command))
		{
			...
			if (command == null)
			{
				if (".rar".equals (file))
				{
					command = "\"unrar\" x -o+ -inul -ri1 -p- -- \"%%PACKEDFILE%%\" \"%%DESTDIR%%\"" + File.separator;
				}
	...
	public void setExpireIncompleteFileCheckIntervalMinutes (int minutes)
	{
		...
		setProperty ("ExpireIncompleteFileCheckIntervalMinutes", String.valueOf (minutes));
	}
	...
	public void setExpirePackageCheckIntervalMinutes (int minutes)
	{
		...
		setProperty ("ExpirePackageCheckIntervalMinutes", String.valueOf (minutes));
	}
	...
	private void notifySettingsChanged()
	{
		...
		while (lis.hasNext())
		{
			SettingsChangedListener listener = (SettingsChangedListener) lis.next();
