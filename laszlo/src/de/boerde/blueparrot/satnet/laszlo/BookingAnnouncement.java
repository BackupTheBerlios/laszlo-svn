...
public class BookingAnnouncement implements Serializable {
		...
		for (int c = 0; c < children.getLength(); c++) {
			...
			if ((child instanceof Element)
				...
				// for (int s=0; s<subchildren.getLength(); s++)
				{
					...
					} else if ((subchild instanceof Text)
							&& (subchild.getNodeType() == Node.TEXT_NODE)) {
	...
	private static String getOneValue(Hashtable hashtable, String remainingPath) {
		...
		if (obj instanceof Vector) {
			...
			for (int v = 0; v < vector.size(); v++) {
				...
				if (!isLast && (part instanceof Hashtable)) {
					String result = getOneValue((Hashtable) part, remainingPath
							.substring(pos + 1));
					if (result != null) {
						return result;
					}
		...
		if (!isLast && (obj instanceof Hashtable)) {
			return getOneValue((Hashtable) obj, remainingPath
					.substring(pos + 1));
		...
		if (value.length() == 12) {
			try {
				...
				GregorianCalendar calendar = new GregorianCalendar(year, month
						- 1 - GregorianCalendar.JANUARY, day, hour, minute, 0);
